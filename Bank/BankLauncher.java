...
public class BankLauncher {
    ...
    public void bankLogin () {
        ...
        Bank searchBank = new Bank(this.idField.getFieldValue(),this.nameField.getFieldValue(),this.passcodeField.getFieldValue());

        int left = 0;
        int right = bankSize() - 1;

        while (left <= right) {
            int mid = left + (right - left) / 2;
            Bank currentBank = getBanks().get(mid);

            // Use the comparator for comparison
            int comparison = this.BankCredentials.compare(searchBank, currentBank);

            if (comparison == 0) {  // Bank found
                this.setLoggedBank(currentBank);
                System.out.println("Login successful! Welcome to " + currentBank.getName());
                return;
    ...
    public void showBanksMenu () {
        ...
        for (Bank banks : this.banks) {
            System.out.print("[" + n + "]   " + banks.getName() + '\n');
            n += 1;
        }
    }
    ...
    public Bank getBank (Comparator < Bank > bankComparator, Bank bank ) {
        ...
        while (l <= r) {
            int m = l + (r - l) / 2;
            int res = bankComparator.compare(getBanks().get(m),bank);
            if (res == 0) {
                return getBanks().get(m);
            } else if (res < 0) {
                l = m + 1;
            } else {
                r = m - 1;
